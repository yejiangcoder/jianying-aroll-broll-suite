...
from aroll_phase4c2_corrected_word_poc import clone_text_material, restore_original_backup, run_post_inspect
...
DEFAULT_PREVIOUS_DIR = Path(r"D:\video tools\jianying-aroll-inspector\runtime\aroll_phase4c3_sentence_gap_poc_20260614_182707")
...
def require_previous_inputs(previous_dir: Path) -> dict[str, Path]:
    names = {
        "input_phase4c3_write_report": "write_report.json",
        "previous_edl": "selected_word_edl_phase4c3.json",
        "previous_subtitles": "selected_subtitle_plan_phase4c3.json",
        "sentence_gap_report": "sentence_gap_compression_report.json",
        "hidden_audio_repeat": "hidden_audio_repeat_diagnostics.json",
    }
    out: dict[str, Path] = {}
    missing: list[str] = []
    for key, name in names.items():
        ...
        out[key] = path
    if missing:
        raise RuntimeError(f"PHASE4C3_INPUT_MISSING:{missing}")
    return out


def copy_inputs(run_dir: Path, paths: dict[str, Path]) -> None:
    shutil.copy2(paths["input_phase4c3_write_report"], run_dir / "input_phase4c3_write_report.json")
...
def write_breath_review(run_dir: Path, plan: dict[str, Any]) -> None:
    lines = [
        "# Intra-segment Breath Cut Plan",
        "",
        f"- breath_cut_count: {plan.get('breath_cut_count')}",
        f"- estimated_removed_pause_s: {plan.get('estimated_removed_pause_s')}",
        f"- average_pause_before_ms: {plan.get('average_pause_before_ms')}",
        f"- average_pause_after_ms: {plan.get('average_pause_after_ms')}",
        "",
        "## Cuts",
    ]
    for row in plan.get("cuts") or []:
        lines.append(
            f"- {row.get('cut_id')} {row.get('cut_type')} removed={round(int(row.get('target_removed_us') or 0)/1000, 3)}ms "
            f"{row.get('left_text')} -> {row.get('right_text')} risk={((row.get('safety') or {}).get('risk'))}"
        )
    (run_dir / "intra_segment_breath_cut_review.md").write_text("\n".join(lines) + "\n", "utf-8")


def write_human_focus(run_dir: Path, audit: dict[str, Any], cut_plan: dict[str, Any], subtitle_plan: list[dict[str, Any]]) -> None:
    lines = [
        "# Phase 4C-4 Human Review Focus",
        "",
        "1. 这版大停顿是否明显继续减少",
        "2. 是否出现新的切字 / 断尾音",
        "3. 字幕是否仍然不碎不乱",
        "4. 评论区重复是否仍然消失",
        "5. 你可以嘲笑他们虚伪是否仍自然",
        "6. 有没有因为切内部 breath 导致跳切感",
        "",
        "## Audit summary",
        f"- detected_pause_count: {audit.get('detected_pause_count')}",
        f"- pause_ge_220ms_count: {audit.get('pause_ge_220ms_count')}",
        f"- breath_cut_count: {cut_plan.get('breath_cut_count')}",
        f"- estimated_removed_pause_s: {cut_plan.get('estimated_removed_pause_s')}",
        "",
        "## Final subtitles",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 4C-4 post-write audio pause audit and breath cut PoC.")
    ...
    run_dir = args.runtime / f"aroll_phase4c4_audio_pause_poc_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    previous_report = read_json(paths["input_phase4c3_write_report"])
    ...
    audit, audit_md = audit_postwrite_audio(previous_edl, previous_subtitles, word_timeline, cutter)
    ...
    cut_plan = build_breath_cut_plan(audit)
    write_json(run_dir / "intra_segment_breath_cut_plan.json", cut_plan)
    write_breath_review(run_dir, cut_plan)

    new_edl = apply_breath_cuts_to_edl(previous_edl, cut_plan)
    new_subtitle_plan = rebase_subtitle_plan(previous_subtitles, new_edl)
    write_json(run_dir / "selected_word_edl_phase4c4.json", new_edl)
    write_json(run_dir / "selected_subtitle_plan_phase4c4.json", new_subtitle_plan)
    ...
    write_human_focus(run_dir, audit, cut_plan, new_subtitle_plan)
    ...
    write_report = {
        ...
        "audio_audit_summary": {
            "virtual_final_audio_generated": audit.get("virtual_final_audio_generated"),
            "virtual_final_audio_scanned": audit.get("virtual_final_audio_scanned"),
            "detected_pause_count": audit.get("detected_pause_count"),
            "pause_ge_140ms_count": audit.get("pause_ge_140ms_count"),
            "pause_ge_220ms_count": audit.get("pause_ge_220ms_count"),
            "type_counts": audit.get("type_counts"),
        },
        "breath_cut_summary": {
            key: cut_plan.get(key)
            for key in [
                "breath_cut_count",
                "inter_clip_compress_count",
                "split_clip_remove_internal_pause_count",
                "trim_clip_edge_count",
                "manual_review_count",
                "estimated_removed_pause_s",
                "average_pause_before_ms",
                "average_pause_after_ms",
            ]
        },
    ...
    print(f"detected_pause_count={audit.get('detected_pause_count')}")
    print(f"breath_cut_count={cut_plan.get('breath_cut_count')}")
    return 0
